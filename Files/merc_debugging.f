...
c
c mal_hvar (time,tstart,tstop,dtout,algor,h0,tol,jcen,
...
c mal_hcon (time,tstart,tstop,dtout,algor,h0,tol,jcen,
c     %  rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,rho,rceh,
c     %  stat,id,ngf,opt,opflag,ngflag,outfile,dumpfile,mem,lmem,onestep,
c     %  coord,bcoord)
c Main integration
...
c AVIMANDELL
c  *Closes output files stranded by comments
      close (21)
      close (22)
      close (23)
      close (24)
...
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
c Author: Avi M. Mandell
c
c Applies drag forces to giant planet to simulate migration, and gas drag
c to small objects.
c
...
c Input/Output
      integer nbod, nbig
      real*8 time,step,m(nbod),x(3,nbod),v(3,nbod)
      real*8 ausr(3,nbod),rphys(NMAX)
c
c Local
      integer j,k
c
      ...
      real*8 rho0,z0,rhogas,mdrag,rdrag,kdrag,ndrag,rhodrag,rhoperc

      real*8 vk,nu,vgas,th,vrel(3,nbod)

      real*8 amig(3,nbod),adrag(3,nbod)
c------------------------------------------------------------------------------
c
c  SETUP
c
      ...
      enddo
c
c  GIANT PLANET MIGRATION DRAG
c
      if (nmig.eq.0) goto 200
      if (aint(time).ge.aint(tmax)) goto 200

      do j = 2,2+nmig
         if (j.gt.1+nmig) goto 199

         R = sqrt(x(1,j)**2 + x(2,j)**2 + x(3,j)**2)
         if (R.le.stopdist) goto 199
         
         if (aint(time).lt.aint(switch*tmax)) then
...
c------------------------------------------------------------------------------
c
c  SMALL OBJECT GAS DRAG
c
      if (rhoconst.eq.0.0d0) goto 300
      if ((gastime.ne.0.0d0).and.(aint(time).ge.aint(gastime))) goto 300

      if (gastime.gt.0.0d0) then
     ...
     %         (((R**2)+(x(3,j)**2))**(3./4.))                        !AU/day
         ...
         if (x(1,j).lt.0.0d0) then
            ...
            if (x(2,j).lt.0.0d0) th = (2.0d0*PI) + th
         endif
         ...
         do k = 1,3
            ...
            if (adrag(k,j)/(-1.0d0*vrel(k,j)/step).gt.1.0d0) 
...
c AVIMANDELL
c      real*8 R0,delR,R,tmig, dummy1
      logical test
c ##R##
	integer loop
...
c ##R## positions should be the same still (this is before the loop)
c	print *, xh(1:3,1:7)
c	print *, vh(1:3,1:7)
	loop=0
...
c AVIMANDELL
c  *Init for the forced migration
c      R0 = sqrt(xh(1,nmig)**2 + xh(2,nmig)**2 + xh(3,nmig)**2)
c
...
c ##R##  check transformed coordinates; xh array only has 6 rows now but x is 3x50000; x=xh, v slightly different than vh
c	print *,shape(xh),shape(x)
c	print *,x(1:3,1:7)
c	print *,v(1:3,1:7)
...
c Main integration step
     ...
     %  mem,lmem)
      time = time + h0
c ##R## Check outputs:
c	print *,time,'d, Row 6 (x) = ',x(1,6)
c AVIMANDELL
c  *Forced migration routine
c      R = sqrt(x(1,nmig)**2 + x(2,nmig)**2 + x(3,nmig)**2)
c      if (aint(time).ge.aint(tmax)) goto 200
c      if ((stopdist.eq.0.0d0).and.(stoptime.eq.0.0d0)) goto 199
c      if ((stopdist.eq.0.0d0).and.(aint(time).lt.aint(stoptime))) 
c     &  goto 199
c      if ((stopdist.eq.0.0d0).and.(aint(time).ge.aint(stoptime))) 
c     &  goto 200
c      if ((stoptime.eq.0.0d0).and.(R.gt.stopdist)) goto 199
c      if ((stoptime.eq.0.0d0).and.(R.le.stopdist)) goto 200
c
c 199  continue
c      if (aint(time).lt.aint(switchtime)) then
c         tmig = tmig0
c      else
c         tmig = tmig0*((tmax-switchtime)/(tmax-time+0.0001))**gamma
c      endif
c      delR = stepdecr*(R0*(h0/tmig))
c
c      x(1,nmig) = x(1,nmig) - (x(1,nmig)*(delR/R))
c      x(2,nmig) = x(2,nmig) - (x(2,nmig)*(delR/R))
c      x(3,nmig) = x(3,nmig) - (x(3,nmig)*(delR/R))
c
c 200  continue
c
c
...
c If something hit the central body, restore the coords prior to this step
      if (nhit.gt.0) then
c ##R## did anything hit?
c	print *, loop,'time=',time, ', nhit= ',nhit, ', jhit=',jhit
...
c Local
      ...
      real*8 hx,hy,hz,h2,p,rr0,rr1,rv0,rv1,vv0,temp,e,v2
      real*8 xu0(3,NMAX),xu1(3,NMAX),vu0(3,NMAX),vu1(3,NMAX)
c
c------------------------------------------------------------------------------
c
c ##R##
c	print *,'mce_cent:',time	
c x0: 3 pos coords for central body (all zero) and each other remaining body
c	print *, x0
      if (i0.le.0) i0 = 2
      nhit = 0
      rcen2 = rcen * rcen
...
c If inside the central body, or passing through pericentre, use 2-body approx.
        if ((rv0*h.le.0.and.rv1*h.ge.0).or.min(rr0,rr1).le.rcen2) then
          ...
          q = p / (1.d0 + e)
c ##R##
c	print *, 'h*sqrt(vv0)=',h*sqrt(vv0),', sqrt(rr0)=',sqrt(rr0)
c	print *,'       loop           j      '
c	print *, loop,j, x0(1,j) , v0(2,j) , x0(2,j) , v0(1,j)
c
...
c Local
      ...
      real*8 ha(2),hb(2),rt10,angf(3,NMAX),ausr(3,NMAX),rphys(NMAX)
c
c------------------------------------------------------------------------------
c
      rt10 = sqrt(10.d0)
...
c If required, apply non-gravitational and user-defined forces
        if (opt(8).eq.1) call mfo_user (time,hb(k),nbod,nbig,m,rphys,xh,
     %    vh,ausr)
...
c
c If required, apply non-gravitational and user-defined forces
        if (opt(8).eq.1) call mfo_user (time,hb(k),nbod,nbig,m,rphys,x,
     %    v,ausr)
...
c If required, apply non-gravitational and user-defined forces
        if (opt(8).eq.1) call mfo_user (time,hby2,nbod,nbig,m,rphys,x,v,
     %    ausr)
        if (ngflag.eq.1.or.ngflag.eq.3) call mfo_ngf (nbod,x,v,angf,ngf)
      endif
c
...
c AVIMANDELL
c  Commented to avoid complicated work
c      if (opt(8).eq.1) then
c        call mfo_user (time,nbod,nbig,m,x,v,acor)
c        do j = 2, nbod
...
c Read integration parameters
  ...
  40    lineno = lineno + 1
	...
	if (j.eq.13.and.(c1.eq.'y'.or.c1.eq.'Y')) opt(5) = 1
...
c
      if (opt(5).eq.1) then
c	open (24, file=outfile(4), status='old', access='append')
	unit = 24
      else
	unit = 6
      endif
